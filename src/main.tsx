    ...
    <CurrencyContext.Provider value={{ currency, setCurrency }}>
      ...
      <header className="topbar" aria-label="Zivo Travel navigation">
        ...
        <div className="utility-nav">
          ...
          <a
            className={`pill icon-only ${supportRoute ? "active" : ""}`}
  ...
  return (
    <a className="feature-card" href={localUrl(searchTabs[0].href)} aria-label="Start booking with Zivo Travel">
      <img
        ...
        src={heroCardImage}
        alt="Zivo Travel booking hero with temple, airplane, and hot-air balloons"
      />
    ...
    () => [
      ...trips.slice(0, 3).map((trip) => {
        ...
        return {
          ...
          // Persistence only means a draft was saved. It is not payment or a
          // provider confirmation, so never turn a local `persisted` flag into
          // a misleading confirmation claim.
          status: trip.status === "confirmed" || trip.status === "provider_confirmed"
            ? "Confirmed"
            : trip.persisted
              ? "Draft"
              : "Resume",
                  ...
                  <div className="trip-row-price">
                    ...
                    <div>
                      <a href={localUrl(trip.reviewUrl)}>
                        {trip.persisted ? "Review secure checkout" : "Resume draft"}
                        <ArrowRight size={16} />
                      </a>
                    </div>
                  </div>
