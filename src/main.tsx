...
function BookingReview({ kind, backendStatus }: { kind: SearchKind; backendStatus: BackendStatus | null }) {
  ...
  async function handleCheckout() {
    // The draft and checkout controls share the same in-flight guard so a second
    // click cannot create a divergent booking reference or navigate before the
    // server has confirmed persistence.
    if (bookingSaving) {
      return;
    }
  ...
  async function prepareDriverRequest() {
    ...
    try {
      ...
      setDriverRequest(null);
      setBookingError("The driver-request preview is unavailable. No driver request was created.");
    } finally {
      setDriverRequestSaving(false);
    }
  }
