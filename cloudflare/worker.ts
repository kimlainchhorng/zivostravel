...
function isServiceRoleSupabaseKey(key: string) {
...
function authorityPublishableKey(env: Env) {
  const key = readEnvSecret(env.ZIVO_AUTHORITY_SUPABASE_PUBLISHABLE_KEY);
  return isServiceRoleSupabaseKey(key) ? "" : key;
}
...
async function createBookingIntent(request: Request, requestUrl: URL, env: Env) {
  ...
  if (!existingResponse.ok) {
    return {
      status: 502,
      payload: { error: "booking_idempotency_lookup_failed", supabaseStatus: existingResponse.status },
    };
  }
