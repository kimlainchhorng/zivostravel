...
/**
 * Booking records belong to the central identity authority. Verify the bearer
 * token with that authority before the Travel service role can read or write
 * anything. A Travel publishable key is never used as a fallback for money-
...
async function requireAuthorityUser(request: Request, env: Env): Promise<AuthorityAuthResult> {
  ...
  try {
    ...
    if (!response.ok) return { ok: false, status: 401, reason: "invalid_authority_session" };
...
async function createBookingIntent(request: Request, requestUrl: URL, env: Env) {
  ...
  if (existingResponse.ok) {
    ...
    if (existing[0]) {
      return { status: 200, payload: bookingResponseFromRow(existing[0], "supabase_booking_idempotent_replay", true) };
    }
  }
  ...
  // A concurrent identical request won the unique (user_id,idempotency_key)
  // insert. Return its canonical record instead of creating a second draft.
  ...
  if (replayResponse.ok) {
    ...
    if (replay[0]) {
      return { status: 200, payload: bookingResponseFromRow(replay[0], "supabase_booking_idempotent_replay", true) };
    }
  }
